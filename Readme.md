...
Данный репозиторий содержит реализацию хэш таблицы с открытой адресацией и двойным хешированием в качестве способа разрешения коллизий.
...
### ***Составляющие хеширования***

Под хешированием будем понимать совокупность 4 основных компонентов:
1. Хэш-таблица
2. Хэш-функция
3. Коллизии
...
+ `Индекс` - значение, возвращаемое хэш-функцией. Соответствует порядковому номеру слота.
...
2. Получить `индекс` в заданном диапазоне, разделив по модулю полученное на предыдущем этапе число на число слотов в таблице. `Индекс` в данном случае соответствует номеру слота, в который будет помещена запись (ключ-значение).
...
`char_code(string[i])` - функция, возвращающая целое число, соответствующее символу. Вместо функции будем использовать ASCII код символа.
...
```
### ***Удаление***
 Поскольку элемент мог участвовать при разрешении коллизий и входить в цепочку слотов, получаемых при двойном хешировании, мы должны сохранить возможность найти элементы, следующие за удаляемым. Для этого указатель на удаляемый элемент заменяется на &HT_DELETED_ITEM= { NULL, NULL }.
```c
/******************************************************
